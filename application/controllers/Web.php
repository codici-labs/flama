...
class Web extends CI_Controller {
	...
	public function index($url = null){
		if(isset($url)){
			...
			$this->db->select('s.name as page_title,s.text as page_content, t.*, s.status_id as status, s.image, u.username');
			...
			if($data['section'] == '' || $data['section']->status == STATUS_DRAFT){
				...
				$data['related'] = $this->getRelatedPost($data['section']->id);
	...
	private function getRelatedPost($post_id){
		$this->db->select('*');
		$this->db->from('sections');
		$this->db->where('category_id', 1);
		$this->db->where('status_id', 2);
